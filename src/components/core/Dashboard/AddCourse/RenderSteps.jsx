...
const RenderSteps = () => {
  ...
  return (
    <>
          ...
          steps.map((item, index) => (
            <>
              ...
              <div className='flex flex-col items-center'>
                ...
                <button className={`${step === item.id
                  ? "bg-yellow-900 border-yellow-50 text-yellow-50"
                  : "border-richblack-700 bg-richblack-800 text-richblack-300"}
                  ${step > item.id && "bg-yellow-50 text-yellow-50"} grid cursor-default place-items-center aspect-square  w-[34px] rounded-full`}>
                  {
      ...
      {/* Render specific components on the basis of current steps */}
